...
(function(){
	...
	RedisDumper = (function(){
		...
		RedisDumper.prototype.dump = function(_arg, callback){
			var convert, db, e, filter, count, format, keys, pretty, ttls, types, values;
			db = _arg.db, filter = _arg.filter, count = _arg.count, format = _arg.format, convert = _arg.convert, pretty = _arg.pretty;
			keys = [];
			types = [];
			values = [];
			ttls = [];
...
//                return _this.db.keys(filter, next);
					...
					return function(replies, next){
						var commands, strcmd, i, item, j, json, k, key, len, ttl, type, v, value, _i, _j, _k, _l, _len, _len1, _len2, _len3, _len4, _len5, _m, _n;
						try{
							for(_i = 0, _len = replies.length; _i < _len; _i++){
								ttl = replies[_i];
								ttls.push(ttl);
									...
									for(i = _k = 0, _len2 = types.length; _k < _len2; i = ++_k){
										...
										switch(type){
											...
											case 'zset':
												...
												commands.push("DEL " + (_this.escape(key)));
												if(value.length !== 0){
													for(j = _n = 0, _len5 = value.length; _n < _len5; j = _n += 2){
														item = value[j];
