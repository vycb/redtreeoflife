		...
		RedisDumper.prototype.dump = function(_arg, callback){
			...
			function getValue(ty, cb){
				...
				if(!!convert){
					...
					multi = this.db.batch();
					for(var i = 0, j = 0, le = types.length; j < le; i = ++j){
						type = types[i];
						switch(type){
							case 'string':
								multi.get(keys[i]);
			...
			function getCommande(tl, cb){
				...
				switch(format){
					case 'json' || 'raw':
						json = {};
						for(var ig = 0, le = 0, lg = types.length; le < lg; ig = ++le){
							type = types[ig];
							key = keys[ig];
							value = values[ig];
							switch(type){
								...
								case 'zset':
									json[key] = {
										...
										value: (function(){
											var _results = [];
											for(var j = 0, le = 0, l = value.length; le < l; j = le += 2){
												item = value[j];
					...
					default:
						commands = "";
						for(var ig = 0, le = 0, lg = types.length; le < lg; le = ++ig){
							type = types[ig];
							key = keys[ig];
							value = values[ig];
							switch(type){
								...
								case 'zset':
									...
									commands += ("DEL " + (this.escape(key))) + "\n";
									if(value.length > 0){
										for(var i = 0, l = 0, le = value.length; l < le; i = l += 2){
											item = value[i];
