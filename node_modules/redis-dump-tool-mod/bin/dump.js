...
(function(){
	...
	RedisDumper = (function(){
		...
		RedisDumper.prototype.dump = function(_arg, callback){
			var e, filter = _arg.filter, numcmd = _arg.numcmd, db = _arg.db, count = _arg.count, format = _arg.format, convert = _arg.convert, pretty = _arg.pretty, keysq = [];
								...
								async.doWhilst(
									function(next){
										_this.db.send_command("scan", [scan, "MATCH", filter, "COUNT", count], function(err, result){
											...
											getType.apply(_this, [keysq, function(err, data){
													if(err)
														console.error(err);
			...
			function getType(ky, cb){
				...
				if(!!convert){
					...
					batch.exec(function(err, result){
						...
						return cb(null,{types: result, keys: keys})
					})
				}
			}

			function getValue(tyo, cb){
				var entry, type, batch, result, val, _ref,
					types = tyo.types, keys = tyo.keys
				if(!!convert){
					result = [];
			...
			function getCommande(tlo, cb){
				var commands, types, strcmd, item, json, key, ttl, type, value, rs = Readable(),
					ttls = tlo, keys = tlo.keys, types = tlo.types, values = tlo.values;
								...
								case 'hash':
									...
									if(Object.keys(value).length > 0){
										...
										for(var k of Object.keys(value)){
											...
											strcmd += " " + (this.escape(k)) + " " + (this.escape(value[k]));
										}
