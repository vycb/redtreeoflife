		...
		RedisDumper.prototype.dump = function(_arg, callback){
			...
			return run([
				(function(_this){
					...
					return function(ky, next){
						var k, key, multi, v, _j, _len1, data = [];
						try{
							keys = ky;
							if(!!convert){
								return next(null, (function(){
								...
								multi = _this.db.batch();
								for(_j = k = 0, _len1 = keys.length; _j < _len1; _j++,k++){
									key = keys[_j];
									multi.type(key);

									if(k > numcmd){
										data = data.concat(multi.exec());
										k = 0;
									}
								}
								function retfu(err, data){
					...
					return function(t, next){
						var entry, type, multi, result, val, _ref, data =[];
						try{
							types = t;
							if(!!convert){
								result = [];
								...
								multi = _this.db.batch();
								for(var i =0, k = 0, l = types.length; i < l; i++,k++){
									type = types[i];
									switch(type){
										case 'string':
											multi.get(keys[i]);
					...
					return function(val, next){
						var key, multi, result, _j, i, k, _len1, _len2, data=[];
						try{
							values = val;
							if(!!convert){
								result = [];
								...
								multi = _this.db.batch();
								for(i = k= 0, _len2 = keys.length; i < _len2; i++,k++){
									key = keys[i];
									multi.ttl(key);
					...
					return function(tl, next){
						var commands, strcmd, i, item, j, json, k, key, len, ttl, type, v, value, _l, _len1, _len2, _len3, _len4, _len5, _mm, rs = Readable();
						try{
							ttls =tl;
							switch(format){
								case 'json' || 'raw':
										...
										switch(type){
											...
											case 'list':
												...
												for(_l = 0, _len3 = value.length; _l < _len3; _l++){
													item = value[_l];
													strcmd += " " + (_this.escape(item));
												}
												commands.push("RPUSH " + (_this.escape(key)) + strcmd);
												break;
											case 'set':
												strcmd = "";
												commands.push("DEL " + (_this.escape(key)));
												if(value.length !== 0){
													...
													commands.push("SADD " + (_this.escape(key)) + strcmd);
												}
												break;
											case 'zset':
												strcmd = "";
												...
												if(value.length !== 0){
													for(j = 0,_len5 = value.length; j < _len5; j +=2){
														item = value[j];
														strcmd += " " + (_this.escape(value[j + 1])) + " " + (_this.escape(value[j]));
													}
													commands.push("ZADD " + (_this.escape(key)) + strcmd);
												}
												break;
											case 'hash':
												strcmd = "";
												...
												if(len !== 0){
													...
													for(k in value){
														v = value[k];
														strcmd += " " + (_this.escape(k)) + " " + (_this.escape(v));
													}
													commands.push("HMSET " + (_this.escape(key)) + strcmd);
												}
										}
										ttl = parseInt(ttls[i], 10);
										if(!isNaN(ttl) && ttl !== -1){
